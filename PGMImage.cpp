...
void PGMImage::set_lbps() {
  if(!(lbps_upper && lbps_lower)) {
    lbps = new uint *[height_];
    for(int i = 0; i < height_; i++){
      lbps[i] = new uint *[width_];
    }
  }
  ...
  for(int search_radius = 0; search_radius <= max_radius; i++){
    ...
    for(auto i = search_list.begin(); i != search_list.end(); i++){
      ...
      if(search_y <= 0 || search_y >= height_ -1)
        continue;
      const uint lbp_upper = lbps[search_x][search_y].first;
      const uint lbp_lower = lbps[search_x][search_y].second;
